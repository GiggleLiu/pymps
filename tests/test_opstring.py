...
class CastTest(object):
    def test_casting2(self):
        ...
        c1=[random.randint(0,config[i]) for i in xrange(4)]
        c2=[random.randint(0,config[i]) for i in xrange(4)]
        c1_=list(c1);c1_[2]=0
        c2_=list(c2);c2_[2]=0
        bl=Bilinear(spaceconfig=scfg,index1=scfg.c2ind(c1),index2=scfg.c2ind(c2))
        ...
        if isinstance(str1,OpString):
            ...
            assert_(str1.opunits[0].factor*(1 if c1[-2]<=c2[-2] else -1)==factor and str1.opunits[1].factor==1.)
            ...
            assert_(str1.factor*(1 if c1[-2]<=c2[-2] else -1)==factor)
    ...
    def test_casting4(self):
        ...
        scfg2=SuperSpaceConfig(config2)
        #test for bilinear casting
        cs=[[random.randint(0,config[i]) for i in xrange(4)] for i in xrange(4)]
        cs_=[]
        for ci in cs:
            ci=list(ci);ci[2]=0
            cs_.append(ci)
        bl=Qlinear(spaceconfig=scfg,indices=array([scfg.c2ind(ci) for ci in cs]))
        indices=[scfg2.c2ind(ci) for ci in cs_]
        str1=xl2string(bl,factor)
        sites=array([c[-2] for c in cs])
        print 'Get sites',sites
        parity=1-2*(sum([sum(sites[:i]>sites[i]) for i in xrange(1,4)])%2)
        print 'parity',parity
        datas=[op_cdag(scfg2,ind)() if i<2 else op_c(scfg2,ind)() for i,ind in enumerate(indices)]
        sites=array([ci[2] for ci in cs])
        datas=array(datas)
        ...
        if isinstance(str1,OpString):
            ...
            for i,ou in enumerate(str1.opunits):
                ...
                if data.shape[0]>1:
                    ...
                    cdata=data[0]
                assert_(ou.factor==(factor*parity if i==0 else 1.))
                assert_allclose(ou.data,cdata,atol=1e-5)
        else:
            print 'Get opunit -> ',[str1]
            assert_(str1.factor*parity==factor)
            assert_allclose(str1.data,datas[0].dot(datas[1]).dot(datas[2]).dot(datas[3]),atol=1e-5)

def test_insertZ():
    scfg=[2,1,1]
    spaceconfig=SuperSpaceConfig(scfg)
    opstring=0.5*opunit_C(dag=True,spaceconfig=spaceconfig,index=0,siteindex=2)*opunit_C(dag=False,spaceconfig=spaceconfig,index=1,siteindex=3)
...
def test_oumul():
    ...
    for oa in [fous[4],bous[4]]:
        for ob in [fous[4],bous[4]]:
            res=oa*ob
            assert_(isinstance(res,OpUnit) and allclose(res.get_data(),oa.get_data().dot(ob.get_data())))
    ...
    for oa in [fous[4],bous[4]]:
        for ob in [fous[3],fous[5],bous[3],bous[5]]:
            ...
            if oa.siteindex>ob.siteindex:
                assert_(isinstance(res,OpString) and allclose(res.opunits[0].get_data(),sign*ob.get_data()) and allclose(res.opunits[1].get_data(),oa.get_data()))
            else:
                assert_(isinstance(res,OpString) and allclose(res.opunits[0].get_data(),sign*oa.get_data()) and allclose(res.opunits[1].get_data(),ob.get_data()))
...
def test_osmul(is_fermion):
    ...
    if is_fermion:
        sign1,sign2,sign3=(-1)**perm_parity(argsort(sites[:nsite/2])),(-1)**perm_parity(argsort(sites[nsite/2:])),(-1)**perm_parity(sites)
        sign4=-sign3
        fous=[random_fop(spaceconfig,siteindex=site) for site in sites]
    else:
    ...
    res3=res1*res2
    res4=OpString(res2.opunits)  #res2 if fermionic, so res2*res1 will have a negative sign with sign3
    res4*=res1
    for i,(res,sign) in enumerate(zip([res1,res2,res3,res4],[sign1,sign2,sign3,sign4])):
        ...
        for ou in res.opunits:
            assert_allclose(fous[list(sites).index(ou.siteindex)].data,ou.data)
        #sign correct
...
def test_add():
    ...
    oss=[ous[i]*ous[i+1] for i in xrange(len(sites)-1)]
    oc1=ous[0]+ous[2]
    oc2=ous[0]+oss[2]
    oc3=oss[0]+oss[2]
    oc4=oc1+oc2+oc3
    assert_(isinstance(oc1,OpCollection) and isinstance(oc2,OpCollection) and isinstance(oc3,OpCollection))
    assert_allclose(oc1.H(nsite=nsite),ous[0].H(nsite=nsite)+ous[2].H(nsite=nsite))
    assert_allclose(oc2.H(nsite=nsite),ous[0].H(nsite=nsite)+oss[2].H(nsite=nsite))
    assert_allclose(oc3.H(nsite=nsite),oss[0].H(nsite=nsite)+oss[2].H(nsite=nsite))
    assert_allclose(oc4.H(nsite=nsite),oc1.H(nsite=nsite)+oc2.H(nsite=nsite)+oc3.H(nsite=nsite))
    ...
    oc2=oc1*ous[1]
    assert_allclose((oc1*oc2+oc2*oc3).H(nsite),(oc1.H(nsite).dot(oc2.H(nsite))+oc2.H(nsite).dot(oc3.H(nsite))))
