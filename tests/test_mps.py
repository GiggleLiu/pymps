...
from tba.hgen import SpinSpaceConfig,quicksave,quickload
from mpo import *
from mpolib import *
from mpslib import *
...
class MPSTest(object):
    ...
    def set_mps(self,i,mps=None,normalize=False):
        ...
        if mps is None:
            ...
            vec=random.random(hndim**nsite)+1j*random.random(hndim**nsite)  #a random state in form of 1D array.
            if normalize:
                vec/=norm(vec)
    ...
    def test_query(self):
        mps=self.mps1
        t0=time.time()
        qu=mps.query((self.hndim-1)*ones(self.nsite))   #query the state of a specified site config.
        t1=time.time()
        vec=mps.state
        print 'Query the last site: %s(true: %s), Elapse -> %s'%(qu,vec[-1],t1-t0)
    ...
    def test_compress(self):
        ...
        print 'State tolerence(after compress, l = %s) %s Elapse -> %s'%(mps.l,sum(abs(nstateadded-vecadded)),t1-t0)
    ...
    def test_addmuldiv(self):
        ...
        t1=time.time()
        nstateadded=mpsadded.state            #recover the 1D array state representation.
        print 'State tolerence(added), %s Elapse -> %s'%(sum(abs(nstateadded-vecadded)),t1-t0)
    ...
    def test_move(self):
        ...
        print 'State tolerence %s, Elapse -> %s'%(sum(abs(nstate-vec)),t1-t0)
        assert_(l2-l1==-4)
        cla();show_mps(mps);pause(sleeptime)
...
def test_prodmps():
    ...
    print 'Get random product state(nsite=%s,hndim=%s) %s'%(nsite,hndim,config)
    ...
    mps=random_product_state(nsite=nsite,hndim=hndim)  #don't know how to test it.
