...
def expect_onsite(opunit,ket,bra=None):
    ...
    if bra is None:
        MH=M.make_copy(labels=[li+'\'' if i==site_axis else li for i,li in enumerate(M.labels)]).conj()
    else:
        MH=bra.get(l,attach_S='B')
        MH.labels=[li+'\'' if i==site_axis else li for i,li in enumerate(M.labels)]
...
def expect_opstring(ket,opstring):
    ...
    for i in xrange(lmin,lmax+1):
        ...
        if i==lmin:
            MH=M.make_copy(labels=[li+'\'' if i in [site_axis,rlink_axis] else li for i,li in enumerate(M.labels)]).conj()
            O=Tensor(opunit.get_data(),labels=[MH.labels[site_axis],M.labels[site_axis]])
            res=MH*O*M
        elif i==lmax:
            MH=M.make_copy(labels=[li+'\'' if i in [site_axis,llink_axis] else li for i,li in enumerate(M.labels)]).conj()
            O=Tensor(opunit.get_data(),labels=[MH.labels[site_axis],M.labels[site_axis]])
            res=res*MH*O*M
        elif i in sites:
            MH=M.make_copy(labels=[li+'\'' for i,li in enumerate(M.labels)]).conj()
            O=Tensor(opunit.get_data(),labels=[MH.labels[site_axis],M.labels[site_axis]])
            res=res*MH*O*M
        else:
            MH=M.make_copy(labels=[li+'\'' if i in [llink_axis,rlink_axis] else li for i,li in enumerate(M.labels)]).conj()
            res=res*MH*M
    return res
...
def get_expect(op,ket,bra=None,sls=slice(None),memorial=False):
    ...
    rlink_axis=ket.rlink_axis
    llink_axis=ket.llink_axis
    if bra is None: bra=ket.tobra()

    res=None
    memory=[]
    attach_S='B' if ket.l==(sls.start if sls.start is not None else 0) else 'A'
    for i in range(ket.nsite)[sls]:
        ...
        if i in sites:
            ...
            MH.labels[site_axis]=M.labels[site_axis]
        items.append(M); items.insert(0,MH)
        for item in items:
...
def get_expect_ivmps_chain(op,ket):
    ...
    res=None
    for sitei in xrange(min(sites),max(sites)+1):
        ti=sitei%ket.npart
        res=ket.tensors[ti]
        if i==lmin:
            #some spetial treatment for first site.
...
def op_mul_mps(op,ket):
    ...
    for i in xrange(nsite):
        ...
        if i in sites:
            opunit=op if isinstance(op,OpUnit) else op.query(i)[0]
            O=Tensor(opunit.get_data(),labels=['x_%s'%i,M.labels[site_axis]])
            M=O*M
            M=M.chorder([1,0,2])
        ML.append(M)
...
class USVobj(LinearOperator):
    ...
    def __init__(self,U,S,V):
        ...
        assert(isinstance(S,Tensor) or (isinstance(S,ndarray) and ndim(S)==1) or (S is None))
        ...
        super(USVobj,self).__init__(shape=(U.shape[0]*U.shape[2],V.shape[0]*V.shape[2]),dtype=U.dtype)

    def __str__(self):
        return 'U(%s|%s)S(%s|%s)V(%s|%s)'%(','.join(self.U.labels),\
                ','.join(str(x) for x in self.U.shape),\
                ','.join(self.S.labels) if isinstance(self.S,Tensor) else '-',\
                ','.join(str(x) for x in self.S.shape) if self.S is not None else 1,\
                ','.join(self.V.labels),\
    ...
    def _matvec(self,x):
        ...
        V=self.V
        x=Tensor(x.reshape(V.shape[0],V.shape[2]),labels=[V.labels[0],V.labels[2]])
        return self.tdot(x)
    ...
    def tdot(self,x):
        ...
        if ndim(res)!=ndim(self.V)+ndim(x)-4:
            raise ValueError('Can not contract V=%s and x=%s properly!'%(self.V.__repr__(),x.__repr__()))
    ...
    def _rmatvec(self,x):
        U=self.U
        x=Tensor(x.reshape(U.shape[0],U.shape[2]),labels=[U.labels[0],U.labels[2]])
        return self.rtdot(x)
    ...
    def rtdot(self,x):
        ...
        if ndim(res)!=ndim(self.U)+ndim(x)-4:
            raise ValueError('Can not contract U=%s and x=%s properly!'%(self.V.__repr__(),x.__repr__()))
        if self.S is not None:
            res=res*self.S
        return res*self.V
    ...
    def _matmat(self,X):
        V=self.V
        X=Tensor(X.reshape(V.shape[0],V.shape[2],X.shape[1]),labels=[V.labels[0],V.labels[2],random.random()])
        return self.tdot(X)

    def _rmatmat(self,X):
        U0=self.usvs[0].U
        U1=self.usvs[-1].U
        X=Tensor(X.reshape(U0.shape[0],U1.shape[2]),labels=[U0.labels[0],U1.labels[2]])
        return self.rtdot(X)
    ...
    def join(self,target):
        ...
        if isinstance(target,USVobj):
            ...
            raise TypeError('Can not join %s and %s'%(self.__class__,target.__class__))
...
class XUSVobj(LinearOperator):
    ...
    def __init__(self,*args):
        ...
        for usv1,usv2 in zip(self.usvs[:-1],self.usvs[1:]):
            assert(usv1.U.shape[2]==usv2.U.shape[0] and usv1.V.shape[2]==usv2.V.shape[0])
            assert(usv1.U.labels[2]==usv2.U.labels[0] and usv1.V.labels[2]==usv2.V.labels[0])
        UL,SL,VL=[],[],[]
    ...
    def _matvec(self,x):
        ...
        x=Tensor(x.reshape([self.usvs[0].V.shape[0],self.usvs[-1].V.shape[2]]),labels=[self.usvs[0].V.labels[0],self.usvs[-1].V.labels[2]])
        return self.tdot(x)
    ...
    def tdot(self,x):
        ...
        VL=[S*V if isinstance(S,Tensor) else (S[:,newaxis]*V if isinstance(S,ndarray) else V) for S,V in zip(SL,VL)]

        for V in VL:
            res=V*res
        if ndim(res)!=self.nusv+resdim:
            raise ValueError('Can not contract V(%s) and x=%s properly!'%(self.shape[1],x.__repr__()))
        ...
        for U in UL:
            res=U*res
        if not reverse:
            res=res.chorder([1,0]+range(2,ndim(res)))
        return res

    def _rmatvec(self,x):
        x=Tensor(x.reshape([self.usvs[0].U.shape[0],self.usvs[-1].U.shape[2]]),labels=[self.usvs[0].U.labels[0],self.usvs[-1].U.labels[2]])
        return self.rtdot(x)
    ...
    def rtdot(self,x):
        ...
        UL=[U*S if isinstance(S,Tensor) else (U*S[:,newaxis] if isinstance(S,ndarray) else U) for S,U in zip(SL,UL)]

        for U in UL:
            res=res*U
        if ndim(res)!=nusv+ndim(x)-2:
            raise ValueError('Can not contract U(%s) and x=%s properly!'%(self.shape[0],x.__repr__()))
        ...
        if reverse:
            resdim=ndim(res)
            res=res.chorder(range(resdim-2)+[resdim-1,resdim-2])
        return res

    def _matmat(self,X):
        V0=self.usvs[0].V
        V1=self.usvs[-1].V
        X=Tensor(X.reshape(V0.shape[0],V1.shape[2],X.shape[1]),labels=[V0.labels[0],V1.labels[2],str(random.random())])
    ...
    def _rmatmat(self,X):
        U0=self.usvs[0].U
        U1=self.usvs[-1].U
        X=Tensor(X.reshape(X.shape[0],U0.shape[0],U1.shape[2]),labels=[str(random.random()),U0.labels[0],U1.labels[2]])
    ...
    def compress(self,d,dlabel='d'):
        ...
        Ut=Tensor(U.reshape([U0.shape[0],U1.shape[-1],d]),labels=[U0.labels[0],U1.labels[-1],dlabel])
        Vt=Tensor(V.reshape([d,V0.shape[0],V1.shape[-1]]),labels=[dlabel,V0.labels[0],V1.labels[-1]])
...
def G_Gong(ket,bra,sls,attach_S='',dense=False,return_histo=False):
    ...
    if bra.labels[0]!=ket.labels[0] or bra.labels[1]==ket.labels[1]:
        raise ValueError('Make site labels identical and bond labels different please!')
    if bra.is_ket or not ket.is_ket:
        raise ValueError()

    MHL,ML=bra.get_all(attach_S=attach_S)[sls],ket.get_all(attach_S=attach_S)[sls]
    res=None
    memory=[]
    for MH,M in zip(MHL,ML):
        if dense:
            ...
            Gi=reduce(add,[kron_csr(csr_matrix(MH[:,i,:]),csr_matrix(M[:,i,:])) for i in xrange(ket.hndim)])
            res=res.dot(Gi) if res is not None else Gi
        memory.append(res)
    if len(ML)==0: raise ValueError()
...
def G_Wang(mpo,ket,bra,sls,attach_S='',dense=False,return_histo=False):
    ...
    res=None
    memory=[]
    MHL,OL,ML=bra.get_all(attach_S=attach_S)[sls],O.get_all()[sls],ket.get_all(attach_S=attach_S)[sls]
