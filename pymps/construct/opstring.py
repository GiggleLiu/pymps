...
class OpUnit(object):
    ...
    def __eq__(self, target):
        ...
        if isinstance(target, OpUnit):
            ...
            raise TypeError('Wrong type for comparison -> %s' %
                            target.__class__)
    ...
    def __mul__(self, target):
        if isinstance(target, numbers.Number):
            if target == 0:
                ...
                res = OpUnit(self.label, self.data, self.siteindex, self.factor,
                             math_str=self.__math_str__, fermionic=self.fermionic)
                res.factor *= target
                return res
            ...
            raise TypeError('Can not multiply %s with %s.' %
    ...
    def __add__(self, target):
        if isinstance(target, OpCollection):
            ...
            raise TypeError('Can not add %s with %s.' %
                            (self.__class__, target.__class__))
    ...
    def __truediv__(self, target):
        if isinstance(target, numbers.Number):
            ...
            raise TypeError('Can not divide %s with %s.' %
                            (self.__class__, target.__class__))
    ...
    def H(self, nsite, dense=True):
        ...
        if dense:
            return kron(kron(identity(hndim**nl), self.get_data(dense=True)), identity(hndim**nr))
        else:
            return sps.kron(sps.kron(sps.identity(hndim**nl), self.get_data(dense=False)), sps.identity(hndim**nr))
...
class OpString():
    ...
    def __add__(self, target):
        if isinstance(target, OpCollection):
            ...
            raise TypeError('OpString not allowed to add with %s.' %
                            target.__class__)
    ...
    def __mul__(self, target):
        if isinstance(target, OpString):
            ...
            raise TypeError('Can not multiply %s with %s(%s).' %
                            (self.__class__, target, target.__class__))
    ...
    def H(self, nsite, dense=True):
        ...
        op_center = identity_(1)
        for i in range(nl, nsite - nr):
            ous = self.query(i)
            if len(ous) == 0:
                ou = one
            elif len(ous) == 1:
            ...
            op_center = kron_(op_center, ou)
        return kron_(kron_(identity_(hndim**nl), op_center), identity_(hndim**nr))
    ...
    def _insert1(self, ou, from_right=True, care_sign=True):
        ...
        if j in sites:
            self.opunits[pos] = (
                self.opunits[pos] * ou) if from_right else (ou * self.opunits[pos])
        else:
            self.opunits.insert(pos, ou)
        if care_sign and ou.fermionic:
            ou_crossed = self.opunits[pos +
                                      1:] if from_right else self.opunits[:pos]
            self *= prod([-1 if oui.fermionic else 1 for oui in ou_crossed])

    # known bug, can not handle the commutation relation.
    def _tonormalorder(self, care_sign=True):
        '''
        ...
        for i in range(nou - 1):
            sites = self.siteindices
            sitei = sites[i]
            minpos = i
            for j in range(i + 1, nou):
...
class OpCollection(object):
    ...
    def __mul__(self, target):
        if isinstance(target, numbers.Number):
            ...
            ops = array([[op * top for top in target.ops]
                         for op in self.ops]).ravel()
            return OpCollection(ops)
        else:
            raise TypeError(
                'Can not multiply OpCollection with %s' % target.__class__)
    ...
    def __imul__(self, target):
        if isinstance(target, numbers.Number):
            ...
            self.ops = array([[op * top for top in target.ops]
                              for op in self.ops]).ravel()
            return self
        else:
            raise TypeError(
    ...
    def __sub__(self, target):
        ...
        if isinstance(target, (OpString, OpUnit)):
            ...
            raise TypeError('Can not subtract %s with %s.' %
                            (self.__class__, target.__class__))
        return opc
    ...
    def H(self, nsite=None, dense=True):
        '''
        ...
        '''
        if nsite is None:
            nsite = self.maxsite + 1
        H = 0
        for opi in self.ops:
            H = H + opi.H(nsite, dense=dense)
        return H
    ...
    def filter(self, func):
        ...
        Returns:
            list, a list of operator string.
        '''
        return [op for op in self.ops if func(op.siteindices)]
...
class OpUnitI(OpUnit):
    ...
    def __init__(self, hndim, siteindex=UNSETTLED):
        super(OpUnitI, self).__init__(label='I', data=identity(
            hndim), siteindex=siteindex, factor=1., math_str='I')
