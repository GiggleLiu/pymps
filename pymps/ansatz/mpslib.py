...
'''
from numpy import *
from scipy.linalg import svd, qr, rq, eigvalsh
...
from ..blockmarker import BlockMarker, trunc_bm
...
def _auto_label(ts, labels, in_place=False, offset=0):
    ...
    res = []
    if ndim(offset) == 0:
        offset = offset * ones(dim)
    for l, t in enumerate(ts):
        assert(ndim(t) == dim)
        lbis = ['%s_%s' % (labels[i], offset[i] + l) for i in range(dim)]
        if in_place:
            t.labels = lbis
        else:
...
def rho_on_link(mps, l=None):
    ...
    if l is None:
        if isinstance(mps, MPS):
            ...
            raise TypeError(
                'Please set division point or use correct <MPS> instance.')
    else:
...
def rho_on_block(vmps, i, j):
    ...
    psi = (ones(1) if i == 0 else vmps.LL[i - 1]
           ).reshape([-1] + [1] * (2 - llink_axis))
    for k in range(i, j):
        rk = vmps.GL[k] * (ones(1) if k == vmps.nsite -
                           1 else vmps.LL[k]).reshape([-1] + [1] * (2 - rlink_axis))
        psi = psi * rk
    aaxes = (llink_axis, rlink_axis + j - i - 1)
    psid = psi.make_copy(
        labels=[l if k in aaxes else l + '\'' for k, l in enumerate(psi.labels)]).conj()
    r = psi * psid
    return r
...
def product_state(config, hndim=None, bmg=None):
    ...
    if ndim(config) == 1:
        if hndim is None:
            raise ValueError('The hndim is needed!')
...
def random_bmps(bmg, nsite, maxN=50):
    ...
    for i in range(nsite):
        ...
        bmi = ts.labels[1].bm
        ts = ts.split_axis(0, nlabels=[bmi_pre, bm1], dims=(
            ts.shape[0] // hndim, hndim))
        bms.append(bmi)
        ML.append(ts)
...
def random_mps(hndim=2, nsite=10, maxN=50):
    ...
    for i in range(nsite):
        ...
        # the factor to make it well normed.
        ts = (random.random([ldim, hndim, rdim]) + 1j *
              random.random([ldim, hndim, rdim])) / sqrt(ldim * rdim)
        ML.append(ts)
...
def check_validity_mps(mps):
    ...
    for i in range(nsite - 1):
        valid = valid and mps.get(
            i + 1).shape[llink_axis] == mps.get(i).shape[rlink_axis]
        if hasattr(mps, 'bmg'):
            valid = valid and mps.get(
                i + 1).labels[llink_axis].bm == mps.get(i).labels[rlink_axis].bm
...
def check_canonical(ket, tol=1e-8):
    ...
    if isinstance(ket, MPS):
        ...
        for i in range(l):
            mi = ket.ML[i]
            res.append(all(abs(sum([mi.take(j, axis=site_axis).T.conj().dot(mi.take(
                j, axis=site_axis)) for j in range(hndim)], axis=0) - identity(mi.shape[ket.rlink_axis])) < tol))
        for i in range(l, ket.nsite):
            mi = ket.ML[i]
            res.append(all(abs(sum([mi.take(j, axis=site_axis).dot(mi.take(j, axis=site_axis).T.conj(
            )) for j in range(hndim)], axis=0) - identity(mi.shape[ket.llink_axis])) < tol))
        return all(res)
    elif isinstance(ket, VidalMPS):
        rl = ket.get_rho()
        # check for left canonical
        i_l = [sum([gi.take(i, axis=site_axis).T.conj().dot(ri).dot(gi.take(i, axis=site_axis))
                    for i in range(hndim)], axis=0) for ri, gi in zip([1] + rl, ket.GL)]
        diff_l = array([sum(abs(ii - identity(ii.shape[-1]))) for ii in i_l])
        # check for right canonical
        i_r = [sum([gi.take(i, axis=site_axis).dot(ri).dot(gi.take(i, axis=site_axis).T.conj())
                    for i in range(hndim)], axis=0) for ri, gi in zip(rl + [1], ket.GL)]
...
def check_flow_mpx(mpx):
    ...
    for i in range(nsite):
        ...
        if isinstance(cell, BTensor):
            bm = cell.labels[0].bm
            valid = valid and all(concatenate(
                [bm.qns[k] for k in list(cell.data.keys())]) == 0)
            continue
...
def swap_forder_advanced(mps, n, forder, spaceconfig, n_start=0, n_end=None, maxN=200):
    ...
    dnl = n - n_start
    dnr = n_end - n
    norder = [o if (o >= n_end or o < n_start) else (
        o + dnr if o < n else o - dnl) for o in forder]
    # get fermionic sign operators.
    ZS = [opunit_Z(spaceconfig, siteindex=j) for j in range(
        nsite) if forder[j] >= n_start and forder[j] < n_end]
    ZL = [opunit_Z(spaceconfig, siteindex=j) for j in range(
        nsite) if forder[j] < n and forder[j] >= n_start]
    ZR = [opunit_Z(spaceconfig, siteindex=j)
          for j in range(nsite) if forder[j] >= n and forder[j] < n_end]
