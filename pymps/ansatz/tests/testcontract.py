...
from ...construct.opstringlib import *
from ...blockmarker import SimpleBMG

from ..mpo import *
from ..mps import BMPS, MPS
from ..mpolib import *
from ..mpslib import *
...
class TestCon(object):
    ...
    def __init__(self):
        ...
        vec = random.random(hndim**nsite)
        # a random state in form of 1D array.
        vec2 = random.random(hndim**nsite)

        # parse the state into a <MPS> instance.
        mps = state2MPS(vec, sitedim=hndim, l=l, method='svd')
        # parse the state into a <MPS> instance.
        mps2 = state2MPS(vec2, sitedim=hndim, l=l, method='svd')
        ...
        WL = [deepcopy(wi) for i in range(nsite)]
        WL[0] = WL[0][3:4]
        WL[-1] = WL[-1][:, :1]
        mpo = WL2MPO(WL)
    ...
    def test_getexpect(self):
        ...
        ops = opunit_Sx(self.spaceconfig).as_site(
            2) * opunit_Sx(self.spaceconfig).as_site(3)
        opc = opunit_Sy(self.spaceconfig).as_site(
            3) * opunit_Sy(self.spaceconfig).as_site(1) + ops
    ...
    def test_usvs(self):
        ...
        for l in [1, 2]:
            ...
            M = U * S[:, newaxis] * V if M is None else U * \
                M * (S[:, newaxis] * V)
            usv = USVobj(U, S, V)
            usvs.append(usv)
        M = M.chorder([1, 0, 2, 3])
    ...
    def test_expect_onsite(self):
        ...
        assert_almost_equal(get_expect(
            opu, mps, bra=mps.tobra(labels=mps.labels)), 1)
...
def test_G_Gong():
    ...
    for sls in [slice(4, 5), slice(None)]:
        ...
        t1 = time.time()
        mat2 = G_Gong(ket, bra, sls, dense=True).chorder(
            [0, 2, 1, 3]).reshape(mat1.shape)
        t2 = time.time()
