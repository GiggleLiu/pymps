...
class VMPSTest(object):
    ...
    def __init__(self):
        ...
        self.vec = random.random(hndim**nsite) / sqrt(hndim**nsite / 2.)
        t0 = time.time()
        self.tol = 4e-2
        # parse the state into a <MPS> instance.
        self.vmps = state2VMPS(self.vec, sitedim=hndim, tol=self.tol)
    ...
    def test_state(self):
        ...
        print('State tolerence %s, Elapse -> %s' %
              (float(sum(abs(nstate - self.vec))), t1 - t0))
    ...
    def test_transform(self):
        ...
        for l in [0, 2, 5, 10]:
            ...
            nstate2 = vmps.state
            print('State tolerence %s, %s, Elapse -> %s, %s' %
                  (sum(abs(nstate - self.vec)), sum(abs(nstate2 - self.vec)), t1 - t0, t2 - t1))
