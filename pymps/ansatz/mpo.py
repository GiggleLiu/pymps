...
from ..blockmarker import block_diag
from ..construct.opstring import OpUnitI, OpUnit
...
def _mpo_sum(mpos, labels=('m', 's', 'b')):
    ...
    for i in range(nsite):
        ai = transpose([[block_diag(*[mi[i][:, j1, j2, :] for mi in MLs])
                         for j2 in range(hndim)] for j1 in range(hndim)], axes=[2, 0, 1, 3])
        ML.append(ai)
    # fix the ends
...
class MPOConstructor(object):
    ...
    def __init__(self, nsite, hndim):
        self.WL = [{} for i in range(nsite)]
        self.links = [set([0]) for i in range(nsite + 1)]
        self.hndim = hndim
    ...
    def read1(self, op):
        ...
        maxsite = sites[-1]
        pre = 0
        for i in range(minsite, maxsite + 1):
            if not i in sites:
    ...
    def compile(self):
        ...
        for sitei, wi in enumerate(self.WL):
            ...
            for i, j in wi:
                ...
                b[maxi + 1, maxj +
                    1] = OpUnitI(hndim=self.hndim, siteindex=sitei)
            WL.append(b)
        WL[0] = WL[0][:1]
        WL[-1] = WL[-1][:, -1:]
        return WL2MPO(WL)
...
class MPOBase(object, metaclass=ABCMeta):
    @abstractmethod
    def get(self, i):
        '''
...
class MPO(MPOBase):
    ...
    def __init__(self, OL, labels=["m", "s", 'b']):
        ...
        for isite, oi in enumerate(OL):
            tlabel = ['%s_%s' % (labels[2], isite), '%s_%s' % (labels[0], isite), '%s_%s' % (
                labels[1], isite), '%s_%s' % (labels[2], isite + 1)]
    ...
    def H(self):
        ...
        H = H.chorder(list(range(1, self.nsite * 2 + 2, 2)) +
                      list(range(0, self.nsite * 2 + 2, 2)))
    ...
    def compress(self, niter=2, tol=1e-8, maxN=Inf, kernel='svd'):
        ...
        acc = 1.
        S = ones(1)
        sweeper = get_sweeper(start=(0, '->', 0),
                              stop=(niter, '<-', 0), nsite=nsite - 2)
        for itervar in sweeper:
            ...
            ckernel = kernel
            if kernel == 'dpl':
                ckernel = ckernel + ('_c' if right else '_r')
            data = AB.svd(cbond=3, cbond_str=cbond_str, bmg=self.bmg if hasattr(
                self, 'bmg') else None, signs=[1, 1, -1, -1, 1, 1], kernel=ckernel)
            U, V = data[0], data[-1]
...
class PMPO(MPOBase):
    ...
    def check_link(self, l):
        l = l % self.nsite
        if l == 0:
            return self.OH[0].shape[0]
        elif l == self.nsite - 1:
            return self.OH[-1].shape[-1]
        else:
...
def OPC2MPO(op, method='direct', nsite=None, bmg=None):
    ...
    if hasattr(op, 'opunits'):
        ...
        if method == 'direct':
            ...
            for i in range(nsite):
                if i == 0:
                    oi = concatenate([mpo.get(i)
                                      for mpo in mpo_collection], axis=3)
                elif i == nsite - 1:
                    oi = concatenate([mpo.get(i)
                                      for mpo in mpo_collection], axis=0)
                else:
                    oi = fmerge_mpo(concatenate(
                        [mpo.get(i) for mpo in mpo_collection], axis=0))
                OL.append(oi)
    else:
