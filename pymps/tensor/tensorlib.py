...
def random_bbtensor(sites=None, labels=None, nnzblock=100):
    ...
    # get entries
    data = dict((tuple(blk), np.random.random(
...
def random_bdmatrix(bm=None, dtype='complex128'):
    ...
    '''
    if dtype == 'complex128':
        cells = [(np.random.random([ni, ni]) + 1j *
                  np.random.random([ni, ni])) * 2 / ni for ni in bm.nr]
    elif dtype == 'float64':
...
def check_flow_tensor(ts, signs, bmg):
    ...
    if isinstance(ts, BTensor):
        bm = ts.labels[0].bm
        return all(concatenate([bm.qns[k] for k in list(ts.data.keys())]) == 0)
    else:
        kpmask = (abs(ts) > 1e-10)
