...
def get_segment(mps, start=None, stop=None, reverse=False):
    ...
    if start is None and stop is None:
        ...
        if reverse:
            ...
            # create a fake site to replace deprecated sites.
            ML, SN = [mps.S[newaxis, :, newaxis]], array([0, mps.nsite])
    elif start is None:
        # the left block.
        if reverse:
        ...
        # create a fake site to replace deprecated sites.
        ML.append(identity(ML[-1].shape[2])[:, :, newaxis])
        ...
        # create a fake site to replace deprecated sites.
        ML.insert(0, identity(ML[0].shape[0])[newaxis, :, :])
        ...
        if not reverse:
            ...
            # create a fake site to replace left sites.
            ML.insert(0, identity(ML[0].shape[0])[newaxis, :, :])
            # create a fake site to replace right sites.
            ML.append((diag(mps.S) if mps.l == stop else identity(
                ML[-1].shape[2]))[:, :, newaxis])
            ...
            M = G_Gong(mps, mps.tobra(
                labels=[mps.labels[0], mps.labels[1] + '\'']), slice(start, stop), attach_S='')
            # then get X that M=X^H*X
            nshape = M.shape
            dim = nshape[0] * nshape[1]
            mat = M.reshape([dim, -1])
            # mat=X^H*X, cholesky is replaced by eigh here for it is rank deficient.
            X = eigen_cholesky(mat)
...
def seg_overlap(ket, bra):
    ...
    '''
    nseg = ket.nseg
    if not (ket.labels[0] == bra.labels[1] and len(set(ket.labels[1:], bra.labels[1:])) == 4):
        raise ValueError()
    ...
    if nseg != 0:
        if ket.Xs[0] is not None:
            ket_segments[0][0] = contract(ket.Xs[0], ket_segment[0][0])
        if ket.Xs[-1] is not None:
            ket_segments[-1][-1] = contract(ket_segment[-1][-1], ket.Xs[-1])
        if bra.Xs[0] is not None:
            bra_segments[0][0] = contract(bra.Xs[0], bra_segment[0][0])
        if bra.Xs[-1] is not None:
            bra_segments[-1][-1] = contract(bra_segment[-1][-1], bra.Xs[-1])
...
def SSFC2E(kets, maxN=30, usvmode=False):
    ...
    edge_labels_l = [bra.ML[bra.l - 1].labels[rlink_axis],
                     ket.ML[l - 1].labels[rlink_axis]]
    ...
    if usvmode:
        ...
        Ci = tensor.Tensor(diag(bra.S), labels=[edge_labels_l[0], edge_labels_r[0]]) * tensor.Tensor(
            diag(ket.S), labels=[edge_labels_l[1], edge_labels_r[1]])
    fs = [1]
    SL = []
    for i in range(ket.nsite / 2):
        t0 = time.time()
        site_l = nsite / 2 - i - 1
        site_r = nsite / 2 + i
        if usvmode:
            t0 = time.time()
            Li = USVobj(U=bra.get(site_l, attach_S='B'),
                        S=None, V=ket.get(site_l, attach_S='B'))
            Ci = Li.join(Ci)
            print(Ci)
            Ci = Ci.compress(min(dl[2 * i + 1], min(Ci.shape)))
            Ri = USVobj(U=bra.get(site_r, attach_S='A'),
                        S=None, V=ket.get(site_r, attach_S='A'))
            Ci = Ci.join(Ri)
            print(Ci)
            t1 = time.time()
            ...
            t0 = time.time()
            Ci = bra.get(site_l, attach_S='B') * (ket.get(site_l, attach_S='B')
                                                  * Ci) * bra.get(site_r, attach_S='A') * ket.get(site_r, attach_S='A')
...
def SSFC2E_F(kets, spaceconfig, maxN=None, usvmode=False):
    ...
    op = prod([opunit_Z(spaceconfig, siteindex=j)
               for j in range(nsite / 2, nsite)])
    ...
    for kt, br in [(ket, bra_ket), (ket_bra, bra)]:
        ...
        for i in range(nsite / 2):
            ...
            O = tensor.Tensor(opunit.get_data(), labels=[
                              MH.labels[site_axis], M.labels[site_axis]])
            items = [MH, O, M]
            for item in items:
...
def SSFE2C(kets):
    ...
    def solve1(l):
        ...
        nsite = ket1.nsite
        t0 = time.time()
        seg1 = get_segment(ket1, start=nsite / 2 - l,
                           end=nsite / 2 + l, reverse=True)
        seg2 = get_segment(ket2, start=nsite / 2 - l,
                           end=nsite / 2 + l, reverse=True)
        t1 = time.time()
...
def seg_overlap(ket, bra, exceptions=[], dense=True):
    ...
    '''
    if not (ket.labels[0] == bra.labels[0] and ket.labels[1] != bra.labels[1:]):
        raise ValueError()
    MHL, ML = [m if i not in exceptions else m.make_copy(
        labels=[m.labels[0], m.labels[1] + '\'', m.labels[2]]) for i, m in enumerate(bra.ML)], ket.ML
    tnet = TNet(MHL + ML)
    ...
    labels = [MHL[0].labels[0]] + [MHL[i].labels[1] for i in exceptions] + [MHL[-1].labels[-1]] +\
        [ML[0].labels[0]] + [ML[i].labels[1]
                             for i in exceptions] + [ML[-1].labels[-1]]
    res = res.chorder([res.labels.index(lbi)
                       for lbi in labels])  # two ends and environments
    return res
