...
def _plot_textedline(pos1,pos2,label,color=None,fontsize=None,ax=None):
    ...
    if color is None: color=legcolor
    if fontsize is None: fontsize=leg_fontsize
...
def _plot_selfloop(pos,label,color=None,fontsize=None,ax=None,zoom=1.):
    ...
    if fontsize is None: fontsize=leg_fontsize
    if ax is None: ax=gca()
    arc=patches.Arc((pos[0], pos[1]+0.34*zoom), width=0.2*zoom, height=0.4*zoom,angle=0, linewidth=legw, fill=False, zorder=-2,color=color)
    ax.add_patch(arc)
...
def draw_rect(x,y,legs={},hatch=None,zoom=1.,ax=None,facetext=None):
    ...
    #show circle
    c1=patches.FancyBboxPatch((x-width/2+pad,y-height/2+pad),width-2*pad,height-2*pad,boxstyle='round,pad=%s'%pad,facecolor=facecolor,hatch=hatch)
    ax.add_patch(c1)

    #draw legs
    for which,leg in legs.iteritems():
        if which=='top':
            dy1=height/2
            dy2=height/2+rv
            dx1,dx2=0,0
        ...
        if which=='top' or which=='bottom':
            x_pos+=(linspace(-5./14*width,5./14*width,nl) if nl>1 else array([0]))
        else:
            y_pos+=(linspace(-5./14*height,5./14*height,nl) if nl>1 else array([0]))
        for pi,(px,py) in enumerate(zip(x_pos,y_pos)):
            plot([px+dx1,px+dx2],[py+dy1,py+dy2],color=legcolor,lw=legw)  #the vertical line
            if labels is not None:
                ...
                if which=='top' or which=='bottom':
                    ...
                    xy[1]-=textoffset
                annotate(str(labels[pi]),xytext=xy,xy=xy,va='center',ha='center',fontsize=leg_fontsize)
...
def draw_tri(x,y,labels=['','',''],legmask=[True]*3,hatch=None,zoom=1.,ax=None,rotate=0):
    ...
    #show triangle
    points=rotate_leg(array([(0,rr),(rr*sqrt(3.)/2,-rr/2.),(-rr*sqrt(3.)/2,-rr/2)]),rotate)+array([x,y])
...
def draw_sq(x,y,labels=['']*4,legmask=[True]*4,hatch=None,zoom=1.,ax=None):
    ...
    #show square
    r1=Rectangle((x-width/2.,y-width/2.),width,height,facecolor=facecolor,hatch=hatch)
    ax.add_patch(r1)
...
def show_mps(mps,offset=(0,0)):
    ...
    for i in xrange(mps.l):
        ...
        text(xi,yi+textoffset,r'$%s$'%mps.ML[i].labels[site_axis],horizontalalignment='center',verticalalignment='center')
        text(xi+rowlink_offset[0],yi+rowlink_offset[1],r'$%s$'%mps.ML[i].labels[llink_axis],horizontalalignment='center',verticalalignment='center')
        text(xi+collink_offset[0],yi+collink_offset[1],r'$%s$'%mps.ML[i].labels[rlink_axis],horizontalalignment='center',verticalalignment='center')
    xS,yS=mps.l+offset[0]-0.5,offset[1]
    plot((xS,xS),(yS-2*r,yS+2*r),color=edgecolor,ls='--')
    text(xS,yS+textoffset,r'$S$',horizontalalignment='center',verticalalignment='center')
    for i in xrange(mps.nsite-mps.l):
        ...
        text(xi,yi+textoffset,r'$%s$'%mps.ML[mps.l+i].labels[site_axis],horizontalalignment='center',verticalalignment='center')
        text(xi+rowlink_offset[0],yi+rowlink_offset[1],r'$%s$'%mps.ML[mps.l+i].labels[llink_axis],horizontalalignment='center',verticalalignment='center')
        text(xi+collink_offset[0],yi+collink_offset[1],r'$%s$'%mps.ML[mps.l+i].labels[rlink_axis],horizontalalignment='center',verticalalignment='center')
    lc=LineCollection(lc,color=edgecolor,lw=2)
    ax.add_collection(lc)
    ax.autoscale()
    axis('equal')
...
def show_opc(opc,nsite=0,offset=(0,0)):
    ...
    for i in xrange(opc.nop):
        ...
        for j,opij in enumerate(opunits):
            ...
            text(x0,y0,opij.get_mathstr(),horizontalalignment='center',verticalalignment='center')
    lc=[]
...
def show_mpo(mpo,offset=(0,0)):
    ...
    for i in xrange(nsite):
        xi,yi=i+offset[0],0+offset[1]
        ax.add_patch(patches.Rectangle(xy=(xi-r,yi-r),width=2*r,height=2*r,facecolor=facecolor,lw=contourlw,edgecolor=edgecolor))
        lc.append([(xi,yi+baroffset),(xi,yi+baroffset+barlen)])
        text(xi,yi+textoffset,r"$%s$"%mpo.OL[i].labels[1],horizontalalignment='center',verticalalignment='center')
        lc.append([(xi,yi-baroffset),(xi,yi-baroffset-barlen)])
        text(xi,yi-textoffset,r"$%s$"%mpo.OL[i].labels[2],horizontalalignment='center',verticalalignment='center')
        text(xi+rowlink_offset[0],yi+rowlink_offset[1],r'$%s$'%mpo.OL[i].labels[0],horizontalalignment='center',verticalalignment='center')
        text(xi+collink_offset[0],yi+collink_offset[1],r'$%s$'%mpo.OL[i].labels[3],horizontalalignment='center',verticalalignment='center')
        if i!=nsite-1:
            lc.append([(xi+r,yi),(xi+1-r,yi)])
    lc=LineCollection(lc,color=edgecolor,lw=2)
    ax.add_collection(lc)
...
def show_opc_advanced(opc,nsite=0,offset=(0,0)):
    ...
    for i in xrange(nsite):
        xi,yi=i+offset[0],0+offset[1]
        ax.add_patch(patches.Circle(xy=(xi,yi),radius=r,facecolor=facecolor,lw=contourlw,edgecolor=edgecolor))
    axis('equal')
    ax.set_xlim(-1,nsite)
...
def show_tnet(tnet,sites,offset=0,dangling_angles=-pi/6):
    ...
    '''
    if len(sites)!=tnet.ntensor: raise ValueError()
    ax=gca()
    leg_fontsize,legw,legcolor,textoffset,leglength=plot_params['leg_fontsize'],plot_params['legw'],\
            plot_params['legcolor'],plot_params['textoffset'],plot_params['leglength']
    sites=sites+offset
    ...
    for i,(tensor,site) in enumerate(zip(tnet.tensors,sites)):
        draw_tensor(site[0],site[1],legs=[],legmask=None,hatch=None,zoom=zoom_tensor,ax=ax,facetext=('$T_%s$'%i,'w',14))

    #draw connections.
    graph=tnet.get_connection()
    for k in xrange(graph.nlink):
    ...
    for i,angle in zip(where(~graph.pairmask)[0],dangling_angles):
        ...
        pos1,pos2=pos0+rotate_leg(dpos1,angle),pos0+rotate_leg(dpos2,angle)
